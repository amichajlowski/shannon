...
- **SDK Integration** — Uses `@anthropic-ai/claude-agent-sdk` with `maxTurns: 10_000` and `bypassPermissions` mode. Adaptive thinking is enabled by default on Opus 4.6/4.7 (`supportsAdaptiveThinking` in `apps/worker/src/ai/models.ts`); disable per-scan via `CLAUDE_ADAPTIVE_THINKING=false` (env) or `core.adaptive_thinking = false` (npx TOML). Browser automation via `playwright-cli` with session isolation (`-s=<session>`). TOTP generation via `generate-totp` CLI tool. Login flow template at `apps/worker/prompts/shared/login-instructions.txt` supports form, SSO, API, and basic auth. On authenticated whitebox scans, the `validate-authentication` preflight performs the single real login and saves the browser session to `auth-state.json` in the per-session audit directory (path from `authStateFile()` in `apps/worker/src/audit/utils.ts`, derived from `generateAuditPath()`). The validation activity (`apps/worker/src/services/validate-authentication.ts`) removes any stale file from a prior run before the agent runs and verifies the file parses and contains cookies or storage before the preflight is marked complete; `logWorkflowComplete` deletes it when the workflow ends so authenticated cookies don't sit on disk between scans. Agent prompts opt in to session reuse by `@include(shared/_shared-session.txt)` before their `<login_instructions>` block — the partial restores the session and falls through to the full login flow if verification fails. `vuln-auth`/`exploit-auth` omit the include and own their own login. Pre-authenticated sessions: the CLI flag `-a`/`--auth-state <file>` (`apps/cli/src/paths.ts:resolveAuthState`, mounted read-only at `/app/auth-state/state.json`) accepts a Playwright `storageState` export captured by the operator (e.g. `npx playwright codegen --save-storage`). It threads through `PipelineInput.authStatePath` → `ActivityInput.authStatePath` → `validateAuthentication`, which injects the file into `authStateFile()` and skips the LLM login entirely (`injectProvidedAuthState`). After adopting the file, the preflight verifies it live (`apps/worker/src/services/verify-injected-session.ts:verifyInjectedSessionLive`): it drives `playwright-cli` (cwd = repo, inheriting the stealth config) to `open` a throwaway `auth-verify` session, `state-load` the injected file, `goto` the `login_url`, and `eval` a probe for the configured `success_condition`. A clean condition-not-met verdict fails the preflight non-retryably (`CONFIG_VALIDATION_FAILED`); transient browser/network problems are retryable (`AGENT_EXECUTION_FAILED`, `network`). Disable the live check via `SHANNON_SKIP_AUTH_STATE_VERIFY=1` (falls back to structural validation only). In this mode `credentials` is optional (`authentication.credentials?` in `apps/worker/src/types/config.ts`; dropped from `required` in `config-schema.json`), but an `authentication` block with `login_url`/`success_condition` is still required so the preflight and agents can verify/restore the session — there is no credential fall-through if it goes stale. Bearer/header APIs: the CLI flag `--auth-header-file <file>` (`apps/cli/src/paths.ts:resolveAuthHeaderFile`, mounted read-only at `/app/auth-header/header.txt`) carries one `Name: Value` header line, captured interactively by `shannon capture-auth` (`apps/cli/src/commands/capture-auth.ts`, which drives host `npx playwright open --save-har` and parses the HAR for the header the frontend sends to the target origin). It threads `PipelineInput.authHeaderFile` → `ActivityInput.authHeaderFile`; `syncPlaywrightStealthConfig` reads it and `writePlaywrightStealthConfig` merges it into `contextOptions.extraHTTPHeaders` plus `network.allowedOrigins` (origin-confined so the token never leaks cross-origin). The preflight `verifyAuthHeader` activity (`apps/worker/src/services/verify-auth-header.ts`) fetches the target and fails fast non-retryably on HTTP 401/403; disable via `SHANNON_SKIP_AUTH_HEADER_VERIFY=1`. Mutually exclusive with `--auth-state` (cookies vs header). Short-lived tokens / long scans: `shannon capture-auth --with-refresh --refresh-url <url>` also seeds a rotating refresh token + target origin into `auth-session.json` (extracted from the captured `storageState` cookies/localStorage). The host-side `shannon auth-proxy` command (`apps/cli/src/commands/auth-proxy.ts`) then runs a refresh loop (POSTs the refresh token to the refresh endpoint just before the JWT `exp`, persisting the rotated token back to the session file) plus a forward proxy that injects `Authorization: Bearer <current>` per request to the target origin. The scan flag `--auth-proxy <url>` threads to `writePlaywrightStealthConfig`, which sets `browser.launchOptions.proxy.server` + `network.allowedOrigins` so all agent traffic routes through it and the token stays fresh across the 1h boundary with no manual re-login (HTTP targets only — no TLS MITM; HTTPS CONNECT is tunneled unmodified). `--auth-state`/`--auth-header-file`/`--auth-proxy` are mutually exclusive
